...
이 스킬은 세션이 끝나기만 하면 최소한의 원본이 자동으로 로컬 커밋까지
됩니다 — 대신 구조가 더 복잡하고(정제 안 된 원본이 `inbox/`에 쌓이고,
그 파일은 git 히스토리에 커밋됨 — gitignore하면 애초에 커밋이 안 되므로
설치 전에 이 저장소 히스토리를 볼 사람 전체를 염두에 두고 판단해야
합니다), `git push`는 자동으로 안 됩니다 (세션마다 무인으로 push하는
훅은 Claude Code auto-mode 분류기가 막아서, 실제 push는 이후 정상적인
작업 흐름에 맡깁니다 — 아무도 안 하면 그 캡처는 로컬 커밋인 채로
사라질 수 있음). 기록하는 습관이 있다면 `durable-session-log`로
충분하고, 그걸 자주 깜빡하거나 세션을 헷갈려서 새로 여는 일이 잦다면
이 스킬을 씁니다. 한 프로젝트에
