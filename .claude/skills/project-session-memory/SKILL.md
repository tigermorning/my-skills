...
기억을 못 한다"는, 이 스킬이 없애려는 문제 그 자체였습니다. 두 스킬이
같은 문제를 다른 방식으로 풀면서 "이번엔 뭘 쓸지" 매번 판단해야 하는
중복이 생겨, `SessionEnd` 훅으로 깜빡함 자체를 구조적으로 없애는 이
스킬 하나로 통합했습니다 (2026-08-30).

기존에 `durable-session-log`를 실제로 설치한 프로젝트(예:
`korean-subtitle-corrector`)는 그대로 둬도 됩니다 — 재설치를 강제하지
않습니다. 다만 앞으로 새 프로젝트에 세션 기억을 적용할 때는 이 스킬을
기본으로 씁니다.
